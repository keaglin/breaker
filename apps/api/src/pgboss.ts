...
// import { getBossInstance } from './pgboss/index';
...
//       try {
//         const id = await boss.send(SUMMARIZE_ENTRY_JOB_NAME, { entryId: entry.id, content: extractTextFromHtml(entry.content!) }, {
...
//   await boss.work<{ entryId: string, content: string }>(SUMMARIZE_ENTRY_JOB_NAME, { batchSize: SUMMARIZE_RATE_LIMIT }, async ([job]) => {
